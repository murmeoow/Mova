...
open class YouTubePlayerSupportFragmentXKt: Fragment(), YouTubePlayer.Provider {
    ...
    fun initialize(youtubeDeveloperKey: String?, initializedListener: YouTubePlayer.OnInitializedListener?, onNavigateBack: () -> Unit,) {
        this.onNavigateBack = onNavigateBack
        initialize(p0 = youtubeDeveloperKey, p1 = initializedListener)
    }
        ...
        this.activity?.onBackPressedDispatcher?.addCallback(
            ...
            onBackPressed = {
                ...
                activity?.supportFragmentManager
                    ?.beginTransaction()
                    ?.remove(this@YouTubePlayerSupportFragmentXKt)
                    ?.commit()
